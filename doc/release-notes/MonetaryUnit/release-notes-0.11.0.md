...
Fortcoin Core 0.11.0 is forked off the Bitcoin Core 0.9.3. The old darkcoin
...
with the introduced changes to the Fortcoin Core 0.11.0.


Fortcoin Core:

- Rebrand to `Fortcoin Core`
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`FORTP`/`FORTV`
  for testnet)
- Adapted BIP44 coin type `5` for Fortcoin (0x80000005) as defined in SLIP-0044
- Added new units: `duffs` (1 / 100.000.000 FORT)
- Added units for testnet: tFORT, mtFORT, utFORT, tduffs
...
- Reduced lower darksend limit to 1.5 FORT
- Fixed progress bar calculation for low amounts
- Improved support for adding FORT after anon has completed
...
- Drop the fee required to relay a transaction to 0.01mFORT per kilobyte
